...
export default async ({params,request, response}: {request:any, params: any, response: any}) => {
  try {
    const {id} = params
    if(!id){
    ...
    const [oldPokemon]: Array<Pokemon> = [...db.query('SELECT name,category,height,weight FROM pokemons WHERE id=?', id).asObjects()]
    ...
    db.query('UPDATE pokemons SET name=?,category=?,height=?,weight=? WHERE id=?', mergedBody.name,mergedBody.category, mergedBody.height, mergedBody.weight, id).return()
