import { BadRequest, HTTPResponse, Ok } from "../types/HTTPResponse.ts";
...
export default async (
  ...
  try {
    ...
    await db.query(
      "INSERT INTO pokemons(name,category,height,weight) VALUES(?,?,?,?)",
      body.name,
      body.category,
      body.height,
      body.weight,
    ...
    response.status = e instanceof HTTPResponse
      ? e.status
      : BadRequest(e.message).status;
  }
};
